...
| :---------: | :---------: | :---------: | :---------: | :---------: | 
| [编程基础](base)|[面向对象基础](#oop)|[泛型编程基础](#gp)|[标准模板库](#stl)|[编译及调试](#other)|
...
5. 结构体内存对齐问题？
    ...
    - 使用#pragma pack(n)设置以n字节对齐，超出n字节长度默认以超出字节长度对齐，#pragma pack()用于取消设置。

    - 类对象的内存对齐同样满足上面原则；union变量长度等于最长的成员长度，union成员共用首地址。
...
6. static作用是什么？在C和C++中有何区别？
 ...
    - static类成员函数：所有对象共享，不含this指针；实现时不需要加static修饰符；不能引用类中声明的非静态成员，可引用类中静态成员。static成员函数可重载。
...
3. 对象存储空间？
 ...
    - 编译器加入的额外成员变量(如指向虚函数的指针)，且虚函数表对应的指针位于该对象地址首部，占4字节，接下来才是数据成员。
...
14. is-a与has-a？
    ...
    - private / protected 继承是has-a。

15. 运算符重载

    - C++中有5种运算符不能重载，分别是：.成员运算符，.*成员指针运算符，::作用域解析运算符，?:条件运算符，sizeof()运算符。
...
1. 模板类与类模板？
    - 类模板允许用户为类定义一种模式，是的类中的某些数据成员，某些成员函数的参数、返回值能够取任意类型，表示的是一个模板，专用于产生类。

    - 模板类是类模板实例化后的一个产物，表示的是由一个模板生成而来的类。
    
---
